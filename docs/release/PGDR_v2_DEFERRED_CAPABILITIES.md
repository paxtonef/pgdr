...
| Real visual dashboard-interpretation provider | B2-V's own founding mandate (and every block since) explicitly authorized a deterministic test stub only — no vision model/API is integrated anywhere; `DashboardInterpretationPort` has zero production implementations | none — a provider integration decision, deliberately out of B2 series scope throughout | UNDECIDED |
| Dashboard capability wiring into SessionController/DiagnosticLoop/CLI | Every B2-D/B2-I mandate deliberately deferred this ("a separate acceptance decision"); wiring an entry point that cannot yet produce a real MATCH (no provider exists) would be dead/misleading code, not a completion — an intentional v1 boundary, not an omission | Real visual dashboard-interpretation provider (above) | v3, or whenever a provider is integrated |
| Additional manufacturer diagnostic-rule authorization (which existing Q&A questions genuinely apply to which manufacturer-backed proposition) | B2-R6 through B2-R10 built and proved the governance MECHANISM (bounded generic-rule inheritance); no additional manufacturer-fact-to-question applicability has been authored or authorized — deliberately, per B2-R9's own "not established != NOT APPLICABLE" epistemic discipline | Automotive domain knowledge authoring (content, not architecture) | v2.x, incremental |
